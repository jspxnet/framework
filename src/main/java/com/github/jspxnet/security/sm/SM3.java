 ...
 * 可满足多种密码应用的安全需求。为了保证杂凑算法的安全性，其产生的杂凑值的长度不应太短，例如MD5输出128比特杂凑值，
 * 输出长度太短，影响其安全性SHA-1算法的输出长度为160比特，SM3算法的输出长度为256比特，
...
public class SM3 {
    ...
    final public static int[] Tj = new int[64];
...
//		System.out.println("");
    ...
    private static byte[] bigEndianIntToByte(int num) {
        return back(EncryptUtil.intToBytes(num));
    }

    private static int bigEndianByteToInt(byte[] bytes) {
        return EncryptUtil.byteToInt(back(bytes));
    }
    ...
    public static byte[] padding(byte[] in, int bLen) {
        ...
        pos += padd.length;
        byte[] tmp = back(EncryptUtil.longToBytes(n));
