...
public class TXWebUtil {
    ...
    public static Object invokeJson(ActionProxy actionProxy) throws Exception {
        ...
        if (methodJson != null && methodJson.containsKey(Environment.rocParams)) {
            ...
            //3.路径名称作为参数
            Object op = methodJson.get(Environment.rocParams);
            if (op instanceof JSONObject) {
                //采用数组方式对于进入
                //自动前后
                JSONObject paramsJson;
                if (methodJson.containsKey(Environment.rocParams)) {
                    paramsJson = methodJson.getJSONObject(Environment.rocParams);
                } else {
                    paramsJson = callJson.getJSONObject(Environment.rocParams);
                }
                paramObj = ParamUtil.getMethodParameter(action, exeMethod, paramsJson);
            } else {
                //采用数组方式对于进入
                JSONArray paramsArray = methodJson.getJSONArray(Environment.rocParams);
