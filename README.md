...
采用resetFull和传统模板呈现页面。自动API文档生成, 其中融入了很多作者开发软件碰到的问题解决方法。
可替代spring cloud构架的的一套解决方案,同时也可以嵌入到spring中是用持久层。 主体结构和spring类似,但功能细节点的设计上不同,更多的添加易性,
...
当前最新版本为 `6.83`，该版本是基于 JDK1.8 的稳定版本，具有以下特性：

- 修复了jsonArray解析的一个bug
- 增强了兼容性和稳定性
- 作为 JDK 1.8 下的长期稳定版本发布
- 后续将开发基于 JDK 21 的新版本

### 版本管理

JSpx Framework 遵循语义化版本控制规范：
...
请查看文档末尾的[发展历程](#发展历程)部分，了解完整的版本更新历史。

### 如何使用

1. 在您的 Maven 项目中添加上述依赖
2. 确保您的项目使用 JDK 1.8 或更高版本
3. 参考 [参考手册](#参考手册) 部分获取更多文档资源
...
* 2025年8月：Jspx cloud Framework 6.82 修复脚本语言复杂表述bug。兼容性和稳定性增强。此版本作为jdk1.8下的稳定版本发布。
* 2026年2月：Jspx cloud Framework 6.83 引入数据库结构元数据结构层，为企业级代码做底层基础,orm部分优化。
* 2026年5月：Jspx cloud Framework 6.84 后期版本将做较大调整。减少实体，架构企业化，融入AI支持。所以这个版本作为基本构架的收官版。后续版本将做较大调整。
